...
export function createTabiTokenKeyManager(keys: string[]): KeyManager {
  return new KeyManager(keys, 'TabiToken');
}

function createAllKeysCooldownError(): any {
  const err: any = new Error('All TabiToken API keys are currently in cooldown. Please wait before retrying.');
  err.status = 429;
...
function wrapError(error: any): any {
  ...
  const msg = errorBody?.error?.message ?? errorBody?.message ?? error?.message ?? 'TabiToken API error';
  const err: any = new Error(`TabiToken API error (${status}): ${msg}`);
  err.status = status;
  err.response = error?.response;
  return err;
}

export class TabiTokenProvider implements Provider {
  ...
  private logSelection(endpoint: string, model: string, keyInfo: KeyInfo): void {
    console.log(`[TABITOKEN] Base URL: ${this.baseUrl}`);
    console.log(`[TABITOKEN] Endpoint: POST ${this.baseUrl}${endpoint}`);
    console.log(`[TABITOKEN] Backend model: ${model}`);
    console.log(`[TABITOKEN] API key index: KEY#${keyInfo.index + 1} (${maskKeySuffix(keyInfo.key)})`);
  }
  ...
  private async executeWithKey<T>(
    ...
    for (let attempt = 0; attempt < this.keyManager.keyCount; attempt++) {
      ...
      try {
        ...
        while (true) {
          try {
            ...
            if (isWafBlock(error) && wafRetry < WAF_RETRY_COUNT) {
              ...
              wafRetry++;
              console.log(`[TABITOKEN][KEY#${keyInfo.index + 1}] WAF blocked  Retry=${wafRetry}/${WAF_RETRY_COUNT}  Backoff=${backoff}ms  Model=${model}`);
              await delay(backoff);
              continue;
            }
            throw error;
        ...
        console.log(`[TABITOKEN][KEY#${keyInfo.index + 1}] Success  Model=${model}  Latency=${latency}ms`);
        return result;
      } catch (error: any) {
        lastError = error;
        const latency = Date.now() - start;
        ...
        if (isQuotaError(error)) {
          ...
          logPipelineRateLimit('tabitoken', model, COOLDOWN_DURATION_MS / 1000, status);
          console.log(`[TABITOKEN][KEY#${keyInfo.index + 1}] Status=${status}  RateLimited  Cooldown=${COOLDOWN_DURATION_MS / 1000}s`);
        } else {
          this.keyManager.markFailure(keyInfo.index, error.message ?? String(error));
          console.log(`[TABITOKEN][KEY#${keyInfo.index + 1}] Failed  Status=${status}  Error=${error.message ?? 'unknown'}`);
        }

        if (attempt < this.keyManager.keyCount - 1 && isRetryableError(error)) {
          console.log(`[TABITOKEN] Retry with fresh key  Attempt=${attempt + 2}  Status=${status}  Model=${model}`);
          logPipelineError({ provider: 'tabitoken', model, status, error: error.message ?? String(error), latencyMs: latency, retry: true });
          continue;
        }

        logPipelineError({ provider: 'tabitoken', model, status, error: error.message ?? String(error), latencyMs: latency });
  ...
  async chatCompletion(payload: any): Promise<any> {
    ...
    logPipelineParsed('tabitoken', payload.model, result, true);
    const { text, location } = findTextInResponse(result);
    logPipelineExtracted('tabitoken', payload.model, text, location);
    logPipelineFinal('tabitoken', payload.model, result);
    return result;
  }
  ...
  async chatCompletionRaw(payload: any): Promise<string> {
    ...
    logPipelineParsed('tabitoken', payload.model, parsed.body, parsed.wasJson, parsed.parseError);
    const { text, location } = findTextInResponse(parsed.body ?? raw);
    logPipelineExtracted('tabitoken', payload.model, text, location);
    logPipelineFinal('tabitoken', payload.model, raw);
    return raw;
  }
  ...
  async chatCompletionStream(payload: any): Promise<{ stream: any; keyIndex: number; tag: string }> {
    ...
    for (let attempt = 0; attempt < this.keyManager.keyCount; attempt++) {
      ...
      try {
        ...
        logSuccessLatency(keyInfo.tag, latency);
        console.log(`[TABITOKEN][KEY#${keyInfo.index + 1}] Success  Model=${model}  Latency=${latency}ms`);
        logPipelineFinal('tabitoken', model, `[streaming] stream established`, latency);
  ...
  async listModels(): Promise<any> {
    ...
    try {
      ...
      console.warn('[TABITOKEN] listModels: no key available — using last-known-good cache');
      const cached = discoveryStore.getLastGoodModels('tabitoken');
      return {
        object: 'list',
        source: cached.length > 0 ? 'cache' : 'fallback',
        data: cached.length > 0 ? cached : FALLBACK_MODELS.map((id) => ({ id, object: 'model', created: Math.floor(now / 1000), owned_by: 'tabitoken' })),
      };
    }
    ...
    const { outcome, models } = await runDiscovery({
      ...
      extract: openAIModelExtractor('tabitoken'),
      onHealthy: (latency) => {
        this.keyManager.markSuccess(keyInfo.index, latency);
        logSuccessLatency(keyInfo.tag, latency);
      },
    ...
    return {
      object: 'list',
      source: 'fallback',
      data: FALLBACK_MODELS.map((id) => ({ id, object: 'model', created: Math.floor(now / 1000), owned_by: 'tabitoken' })),
    };
  }
  ...
  async healthCheck(): Promise<any> {
    ...
    try {
      ...
      return {
        ...
        models: 0,
        error: 'All TabiToken API keys are currently in cooldown',
      };
    }
  ...
  async createEmbedding(_payload: any): Promise<any> {
    const err: any = new Error('TabiToken provider does not support embeddings.');
    err.status = 400;
    throw err;
  }
  ...
  private async makeRequest(method: string, url: string, data: any, apiKey: string, extraConfig?: AxiosRequestConfig): Promise<any> {
    ...
    logPipelineRequest({ provider: 'tabitoken', baseUrl: this.baseUrl, endpoint: url, model, protocol: PROTOCOL, keyMasked: maskApiKey(apiKey) });
    const start = Date.now();
    ...
    logPipelineRaw({ provider: 'tabitoken', model, endpoint: url, protocol: PROTOCOL, status: response.status, headers: response.headers, body: response.data, latencyMs: Date.now() - start });
  ...
  private async makeStreamRequest(url: string, data: any, apiKey: string): Promise<any> {
    ...
    logPipelineRaw({ provider: 'tabitoken', model, endpoint: url, protocol: PROTOCOL, status: response.status, headers: response.headers, body: '[streaming]', latencyMs: Date.now() - start });
    return response.data;
  }
}