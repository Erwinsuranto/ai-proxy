...
const PROVIDER_INFO: ProviderInfo = {
  providerId: 'hashneuron',
  providerName: 'HashNeuron',
};

// Upstream endpoints (relative to baseUrl, e.g. https://hashneuron.space/v1).
...
export function createHashNeuronKeyManager(keys: string[]): KeyManager {
  return new KeyManager(keys, 'HashNeuron');
}

function createAllKeysCooldownError(): any {
  const err: any = new Error('All HashNeuron API keys are currently in cooldown. Please wait before retrying.');
  err.status = 429;
...
function wrapError(error: any): any {
  ...
  const msg = errorBody?.error?.message ?? errorBody?.message ?? error?.message ?? 'HashNeuron API error';
  const err: any = new Error(`HashNeuron API error (${status}): ${msg}`);
  err.status = status;
  err.response = error?.response;
  return err;
}

export class HashNeuronProvider implements Provider {
  ...
  private logSelection(endpoint: string, model: string, keyInfo: KeyInfo): void {
    console.log(`[HASHNEURON] Base URL: ${this.baseUrl}`);
    console.log(`[HASHNEURON] Endpoint: POST ${this.baseUrl}${endpoint}`);
    console.log(`[HASHNEURON] Backend model: ${model}`);
    console.log(`[HASHNEURON] API key index: KEY#${keyInfo.index + 1} (${maskKeySuffix(keyInfo.key)})`);
  }
  ...
  private async executeWithKey<T>(
    ...
    try {
      ...
      console.log(`[HASHNEURON][KEY#${keyInfo.index + 1}] Success  Model=${model}  Latency=${latency}ms`);
      return result;
    } catch (error: any) {
      const latency = Date.now() - start;
      ...
      if (isQuotaError(error)) {
        ...
        logPipelineRateLimit('hashneuron', model, COOLDOWN_DURATION_MS / 1000, status);
        console.log(`[HASHNEURON][KEY#${keyInfo.index + 1}] Status=${status}  RateLimited  Cooldown=${COOLDOWN_DURATION_MS / 1000}s`);
      } else {
        this.keyManager.markFailure(keyInfo.index, error.message ?? String(error));
        console.log(`[HASHNEURON][KEY#${keyInfo.index + 1}] Failed  Status=${status}  Error=${error.message ?? 'unknown'}`);
      }
      logPipelineError({ provider: 'hashneuron', model, status, error: error.message ?? String(error), latencyMs: latency });
  ...
  async chatCompletion(payload: any): Promise<any> {
    ...
    logPipelineParsed('hashneuron', payload.model, result, true);
    const { text, location } = findTextInResponse(result);
    logPipelineExtracted('hashneuron', payload.model, text, location);
    logPipelineFinal('hashneuron', payload.model, result);
    return result;
  }
  ...
  async chatCompletionRaw(payload: any): Promise<string> {
    ...
    logPipelineParsed('hashneuron', payload.model, parsed.body, parsed.wasJson, parsed.parseError);
    const { text, location } = findTextInResponse(parsed.body ?? raw);
    logPipelineExtracted('hashneuron', payload.model, text, location);
    logPipelineFinal('hashneuron', payload.model, raw);
    return raw;
  }
  ...
  async chatCompletionStream(payload: any): Promise<{ stream: any; keyIndex: number; tag: string }> {
    ...
    try {
      ...
      logSuccessLatency(keyInfo.tag, latency);
      console.log(`[HASHNEURON][KEY#${keyInfo.index + 1}] Success  Model=${model}  Latency=${latency}ms`);
      logPipelineFinal('hashneuron', model, `[streaming] stream established`, latency);
  ...
  // --- Static model catalog (no live discovery) ---
  // Mendaftarkan katalog model dari GET /v1/models upstream
  // (RouteOpen gateway di https://hashneuron.space/v1).

  async listModels(): Promise<any> {
    const now = Date.now();
    const data = FALLBACK_MODELS.map((id) => ({ id, object: 'model', created: Math.floor(now / 1000), owned_by: 'hashneuron' }));
  ...
  async healthCheck(): Promise<any> {
    ...
    try {
      ...
      return {
        ...
        models: 0,
        error: 'All HashNeuron API keys are currently in cooldown',
      };
    }
  ...
  async createEmbedding(_payload: any): Promise<any> {
    const err: any = new Error('HashNeuron provider does not support embeddings.');
    err.status = 400;
    throw err;
  }
  ...
  private async makeRequest(method: string, url: string, data: any, apiKey: string, extraConfig?: AxiosRequestConfig): Promise<any> {
    ...
    logPipelineRequest({ provider: 'hashneuron', baseUrl: this.baseUrl, endpoint: url, model, protocol: PROTOCOL, keyMasked: maskApiKey(apiKey) });
    const start = Date.now();
    ...
    logPipelineRaw({ provider: 'hashneuron', model, endpoint: url, protocol: PROTOCOL, status: response.status, headers: response.headers, body: response.data, latencyMs: Date.now() - start });
    // Fix #1: HTTP 200 but the body is actually an inline upstream error.
    const inline = detectInlineUpstreamError(response.data);
    if (inline) {
      throw createUpstreamError('HashNeuron', inline);
    }
  ...
  private async makeStreamRequest(url: string, data: any, apiKey: string): Promise<any> {
    ...
    const response = await this.client.post(url, data, {
      headers: this.buildHeaders(apiKey),
      responseType: 'stream',
      // Fix #2: HashNeuron requests must not be cut off at the global 30s timeout.
      // Apply the HashNeuron-specific (120s) timeout consistently to normal AND
      // streaming requests so long generations can complete.
      timeout: this.timeout,
    } as AxiosRequestConfig);
    logPipelineRaw({ provider: 'hashneuron', model, endpoint: url, protocol: PROTOCOL, status: response.status, headers: response.headers, body: '[streaming]', latencyMs: Date.now() - start });
  ...
  private inspectStreamStart(upstream: any): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      const settle = (verdict: StreamVerdict) => {
        ...
        cleanup();
        if (verdict === 'error') {
          const err = createUpstreamError('HashNeuron', {
            status: 502,
