...
// Upstream endpoints (relative to baseUrl, e.g. https://api.hcnsec.cn/v1).
...
export function createHCNSecKeyManager(keys: string[]): KeyManager {
  return new KeyManager(keys, 'HCNSec');
}

function createAllKeysCooldownError(): any {
  const err: any = new Error('All HCNSec API keys are currently in cooldown. Please wait before retrying.');
  err.status = 429;
...
function wrapError(error: any): any {
  ...
  const msg = errorBody?.error?.message ?? errorBody?.message ?? error?.message ?? 'HCNSec API error';
  const err: any = new Error(`HCNSec API error (${status}): ${msg}`);
  err.status = status;
  err.response = error?.response;
  return err;
}

export class HCNSecProvider implements Provider {
  ...
  private logSelection(endpoint: string, model: string, keyInfo: KeyInfo): void {
    console.log(`[HCNSEC] Base URL: ${this.baseUrl}`);
    console.log(`[HCNSEC] Endpoint: POST ${this.baseUrl}${endpoint}`);
    console.log(`[HCNSEC] Backend model: ${model}`);
    console.log(`[HCNSEC] API key index: KEY#${keyInfo.index + 1} (${maskKeySuffix(keyInfo.key)})`);
  }
  ...
  private async executeWithKey<T>(
    ...
    for (let attempt = 0; attempt < this.keyManager.keyCount; attempt++) {
      ...
      try {
        ...
        console.log(`[HCNSEC][KEY#${keyInfo.index + 1}] Success  Model=${model}  Latency=${latency}ms`);
        return result;
      } catch (error: any) {
        const latency = Date.now() - start;
        ...
        if (isQuotaError(error)) {
          ...
          logPipelineRateLimit('hcnsec', model, COOLDOWN_DURATION_MS / 1000, status);
          console.log(`[HCNSEC][KEY#${keyInfo.index + 1}] Status=${status}  RateLimited  Cooldown=${COOLDOWN_DURATION_MS / 1000}s`);
        } else {
          this.keyManager.markFailure(keyInfo.index, error.message ?? String(error));
          console.log(`[HCNSEC][KEY#${keyInfo.index + 1}] Failed  Status=${status}  Error=${error.message ?? 'unknown'}`);
        }
        logPipelineError({ provider: 'hcnsec', model, status, error: error.message ?? String(error), latencyMs: latency });
        lastError = wrapError(error);
      }
    }
    throw lastError ?? wrapError(new Error('All HCNSec API keys failed'));
  }
  ...
  async chatCompletion(payload: any): Promise<any> {
    ...
    logPipelineParsed('hcnsec', payload.model, result, true);
    const { text, location } = findTextInResponse(result);
    logPipelineExtracted('hcnsec', payload.model, text, location);
    logPipelineFinal('hcnsec', payload.model, result);
    return result;
  }
  ...
  async chatCompletionRaw(payload: any): Promise<string> {
    ...
    logPipelineParsed('hcnsec', payload.model, parsed.body, parsed.wasJson, parsed.parseError);
    const { text, location } = findTextInResponse(parsed.body ?? raw);
    logPipelineExtracted('hcnsec', payload.model, text, location);
    logPipelineFinal('hcnsec', payload.model, raw);
    return raw;
  }
  ...
  async chatCompletionStream(payload: any): Promise<{ stream: any; keyIndex: number; tag: string }> {
    ...
    for (let attempt = 0; attempt < this.keyManager.keyCount; attempt++) {
      ...
      try {
        ...
        logSuccessLatency(keyInfo.tag, latency);
        console.log(`[HCNSEC][KEY#${keyInfo.index + 1}] Success  Model=${model}  Latency=${latency}ms`);
        logPipelineFinal('hcnsec', model, `[streaming] stream established`, latency);
  ...
  async listModels(): Promise<any> {
    ...
    try {
      ...
      console.warn('[HCNSEC] listModels: no key available — using last-known-good cache');
      const cached = discoveryStore.getLastGoodModels('hcnsec');
      return {
        object: 'list',
        source: cached.length > 0 ? 'cache' : 'fallback',
        data: cached.length > 0 ? cached : FALLBACK_MODELS.map((id) => ({ id, object: 'model', created: Math.floor(now / 1000), owned_by: 'hcnsec' })),
      };
    }
    ...
    const { outcome, models } = await runDiscovery({
      ...
      extract: openAIModelExtractor('hcnsec'),
      onHealthy: (latency) => {
        this.keyManager.markSuccess(keyInfo.index, latency);
        logSuccessLatency(keyInfo.tag, latency);
      },
    ...
    return {
      object: 'list',
      source: 'fallback',
      data: FALLBACK_MODELS.map((id) => ({ id, object: 'model', created: Math.floor(now / 1000), owned_by: 'hcnsec' })),
    };
  }
  ...
  async healthCheck(): Promise<any> {
    ...
    try {
      ...
      return {
        ...
        models: 0,
        error: 'All HCNSec API keys are currently in cooldown',
      };
    }
  ...
  async createEmbedding(_payload: any): Promise<any> {
    const err: any = new Error('HCNSec provider does not support embeddings.');
    err.status = 400;
    throw err;
  }
  ...
  private async makeRequest(method: string, url: string, data: any, apiKey: string, extraConfig?: AxiosRequestConfig): Promise<any> {
    ...
    logPipelineRequest({ provider: 'hcnsec', baseUrl: this.baseUrl, endpoint: url, model, protocol: PROTOCOL, keyMasked: maskApiKey(apiKey) });
    const start = Date.now();
    ...
    logPipelineRaw({ provider: 'hcnsec', model, endpoint: url, protocol: PROTOCOL, status: response.status, headers: response.headers, body: response.data, latencyMs: Date.now() - start });
  ...
  private async makeStreamRequest(url: string, data: any, apiKey: string): Promise<any> {
    ...
    logPipelineRaw({ provider: 'hcnsec', model, endpoint: url, protocol: PROTOCOL, status: response.status, headers: response.headers, body: '[streaming]', latencyMs: Date.now() - start });
    return response.data;
  }
}