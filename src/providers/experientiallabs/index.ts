...
const PROVIDER_INFO: ProviderInfo = {
  ...
  providerName: 'ExperientialLabs',
};

// Upstream endpoints (relative to baseUrl, e.g. https://api.experientiallabs.ai/v1).
...
export function createExperientialLabsKeyManager(keys: string[]): KeyManager {
  return new KeyManager(keys, 'ExperientialLabs');
}

function createAllKeysCooldownError(): any {
  const err: any = new Error('All ExperientialLabs keys are currently in cooldown. Please wait before retrying.');
  err.status = 429;
...
function wrapError(error: any): any {
  ...
  const msg = errorBody?.error?.message ?? errorBody?.message ?? error?.message ?? 'ExperientialLabs error';
  const err: any = new Error(`ExperientialLabs API error (${status}): ${msg}`);
  err.status = status;
  err.response = error?.response;
  return err;
}

export class ExperientialLabsProvider implements Provider {
  ...
  private logSelection(endpoint: string, model: string, keyInfo: KeyInfo): void {
    console.log(`[EXPERIENTIALLABS] Base URL: ${this.baseUrl}`);
    console.log(`[EXPERIENTIALLABS] Endpoint: POST ${this.baseUrl}${endpoint}`);
    console.log(`[EXPERIENTIALLABS] Backend model: ${model}`);
    console.log(`[EXPERIENTIALLABS] API key index: KEY#${keyInfo.index + 1} (${maskKeySuffix(keyInfo.key)})`);
  }
  ...
  private async executeWithKey<T>(
    ...
    try {
      ...
      console.log(`[EXPERIENTIALLABS][KEY#${keyInfo.index + 1}] Success  Model=${model}  Latency=${latency}ms`);
      return result;
    } catch (error: any) {
      const latency = Date.now() - start;
      ...
      if (isQuotaError(error)) {
        ...
        logPipelineRateLimit('experientiallabs', model, COOLDOWN_DURATION_MS / 1000, status);
        console.log(`[EXPERIENTIALLABS][KEY#${keyInfo.index + 1}] Status=${status}  RateLimited  Cooldown=${COOLDOWN_DURATION_MS / 1000}s`);
      } else {
        this.keyManager.markFailure(keyInfo.index, error.message ?? String(error));
        console.log(`[EXPERIENTIALLABS][KEY#${keyInfo.index + 1}] Failed  Status=${status}  Error=${error.message ?? 'unknown'}`);
      }
      logPipelineError({ provider: 'experientiallabs', model, status, error: error.message ?? String(error), latencyMs: latency });
  ...
  async chatCompletion(payload: any): Promise<any> {
    ...
    logPipelineParsed('experientiallabs', payload.model, result, true);
    const { text, location } = findTextInResponse(result);
    logPipelineExtracted('experientiallabs', payload.model, text, location);
    logPipelineFinal('experientiallabs', payload.model, result);
    return result;
  }
  ...
  async chatCompletionRaw(payload: any): Promise<string> {
    ...
    logPipelineParsed('experientiallabs', payload.model, parsed.body, parsed.wasJson, parsed.parseError);
    const { text, location } = findTextInResponse(parsed.body ?? raw);
    logPipelineExtracted('experientiallabs', payload.model, text, location);
    logPipelineFinal('experientiallabs', payload.model, raw);
    return raw;
  }
  ...
  async chatCompletionStream(payload: any): Promise<{ stream: any; keyIndex: number; tag: string }> {
    ...
    try {
      ...
      logSuccessLatency(keyInfo.tag, latency);
      console.log(`[EXPERIENTIALLABS][KEY#${keyInfo.index + 1}] Success  Model=${model}  Latency=${latency}ms`);
      logPipelineFinal('experientiallabs', model, `[streaming] stream established`, latency);
  ...
  async listModels(): Promise<any> {
    ...
    try {
      ...
      console.warn('[EXPERIENTIALLABS] listModels: no key available — using last-known-good cache');
      const cached = discoveryStore.getLastGoodModels('experientiallabs');
      return {
        object: 'list',
        source: cached.length > 0 ? 'cache' : 'fallback',
        data: cached.length > 0 ? cached : FALLBACK_MODELS.map((id) => ({ id, object: 'model', created: Math.floor(now / 1000), owned_by: 'experientiallabs' })),
      };
    }
    ...
    const { outcome, models } = await runDiscovery({
      ...
      extract: openAIModelExtractor('experientiallabs'),
      onHealthy: (latency) => {
        this.keyManager.markSuccess(keyInfo.index, latency);
        logSuccessLatency(keyInfo.tag, latency);
      },
    ...
    const mergeFallback = (apiModels: any[]): any[] => {
      ...
      for (const id of FALLBACK_MODELS) {
        if (!apiIds.has(id.toLowerCase())) {
          apiModels.push({ id, object: 'model', created: nowSec, owned_by: 'experientiallabs' });
        }
      }
      return apiModels;
    };
    ...
    return {
      object: 'list',
      source: 'fallback',
      data: FALLBACK_MODELS.map((id) => ({ id, object: 'model', created: Math.floor(now / 1000), owned_by: 'experientiallabs' })),
    };
  }
  ...
  async healthCheck(): Promise<any> {
    ...
    try {
      ...
      return {
        ...
        models: 0,
        error: 'All ExperientialLabs keys are currently in cooldown',
      };
    }
  ...
  async createEmbedding(_payload: any): Promise<any> {
    const err: any = new Error('ExperientialLabs provider does not support embeddings.');
    err.status = 400;
    throw err;
  }
  ...
  private async makeRequest(method: string, url: string, data: any, apiKey: string, extraConfig?: AxiosRequestConfig): Promise<any> {
    ...
    logPipelineRequest({ provider: 'experientiallabs', baseUrl: this.baseUrl, endpoint: url, model, protocol: PROTOCOL, keyMasked: maskApiKey(apiKey) });
    const start = Date.now();
    ...
    logPipelineRaw({ provider: 'experientiallabs', model, endpoint: url, protocol: PROTOCOL, status: response.status, headers: response.headers, body: response.data, latencyMs: Date.now() - start });
  ...
  private async makeStreamRequest(url: string, data: any, apiKey: string): Promise<any> {
    ...
    logPipelineRaw({ provider: 'experientiallabs', model, endpoint: url, protocol: PROTOCOL, status: response.status, headers: response.headers, body: '[streaming]', latencyMs: Date.now() - start });
    return response.data;
  }
}
