...
// Upstream endpoints (relative to baseUrl, e.g. https://api.atria-asi.ai/v1).
...
export function createAtriaKeyManager(keys: string[]): KeyManager {
  return new KeyManager(keys, 'Atria');
}

function createAllKeysCooldownError(): any {
  const err: any = new Error('All Atria keys are currently in cooldown. Please wait before retrying.');
  err.status = 429;
...
function wrapError(error: any): any {
  ...
  const msg = errorBody?.error?.message ?? errorBody?.message ?? error?.message ?? 'Atria error';
  const err: any = new Error(`Atria API error (${status}): ${msg}`);
  err.status = status;
  err.response = error?.response;
  return err;
}

export class AtriaProvider implements Provider {
  ...
  private logSelection(endpoint: string, model: string, keyInfo: KeyInfo): void {
    console.log(`[ATRIA] Base URL: ${this.baseUrl}`);
    console.log(`[ATRIA] Endpoint: POST ${this.baseUrl}${endpoint}`);
    console.log(`[ATRIA] Backend model: ${model}`);
    console.log(`[ATRIA] API key index: KEY#${keyInfo.index + 1} (${maskKeySuffix(keyInfo.key)})`);
  }
  ...
  private async executeWithKey<T>(
    ...
    try {
      ...
      console.log(`[ATRIA][KEY#${keyInfo.index + 1}] Success  Model=${model}  Latency=${latency}ms`);
      return result;
    } catch (error: any) {
      const latency = Date.now() - start;
      ...
      if (isQuotaError(error)) {
        ...
        logPipelineRateLimit('atria', model, COOLDOWN_DURATION_MS / 1000, status);
        console.log(`[ATRIA][KEY#${keyInfo.index + 1}] Status=${status}  RateLimited  Cooldown=${COOLDOWN_DURATION_MS / 1000}s`);
      } else {
        this.keyManager.markFailure(keyInfo.index, error.message ?? String(error));
        console.log(`[ATRIA][KEY#${keyInfo.index + 1}] Failed  Status=${status}  Error=${error.message ?? 'unknown'}`);
      }
      logPipelineError({ provider: 'atria', model, status, error: error.message ?? String(error), latencyMs: latency });
  ...
  async chatCompletion(payload: any): Promise<any> {
    ...
    logPipelineParsed('atria', payload.model, result, true);
    const { text, location } = findTextInResponse(result);
    logPipelineExtracted('atria', payload.model, text, location);
    logPipelineFinal('atria', payload.model, result);
    return result;
  }
  ...
  async chatCompletionRaw(payload: any): Promise<string> {
    ...
    logPipelineParsed('atria', payload.model, parsed.body, parsed.wasJson, parsed.parseError);
    const { text, location } = findTextInResponse(parsed.body ?? raw);
    logPipelineExtracted('atria', payload.model, text, location);
    logPipelineFinal('atria', payload.model, raw);
    return raw;
  }
  ...
  async chatCompletionStream(payload: any): Promise<{ stream: any; keyIndex: number; tag: string }> {
    ...
    try {
      ...
      logSuccessLatency(keyInfo.tag, latency);
      console.log(`[ATRIA][KEY#${keyInfo.index + 1}] Success  Model=${model}  Latency=${latency}ms`);
      logPipelineFinal('atria', model, `[streaming] stream established`, latency);
  ...
  async listModels(): Promise<any> {
    ...
    try {
      ...
      console.warn('[ATRIA] listModels: no key available — using last-known-good cache');
      const cached = discoveryStore.getLastGoodModels('atria');
      return {
        object: 'list',
        source: cached.length > 0 ? 'cache' : 'fallback',
        data: cached.length > 0 ? cached : FALLBACK_MODELS.map((id) => ({ id, object: 'model', created: Math.floor(now / 1000), owned_by: 'atria' })),
      };
    }
    ...
    const { outcome, models } = await runDiscovery({
      ...
      extract: openAIModelExtractor('atria'),
      onHealthy: (latency) => {
        this.keyManager.markSuccess(keyInfo.index, latency);
        logSuccessLatency(keyInfo.tag, latency);
      },
    ...
    const mergeFallback = (apiModels: any[]): any[] => {
      ...
      for (const id of FALLBACK_MODELS) {
        if (!apiIds.has(id.toLowerCase())) {
          apiModels.push({ id, object: 'model', created: nowSec, owned_by: 'atria' });
        }
      }
      return apiModels;
    };
    ...
    return {
      object: 'list',
      source: 'fallback',
      data: FALLBACK_MODELS.map((id) => ({ id, object: 'model', created: Math.floor(now / 1000), owned_by: 'atria' })),
    };
  }
  ...
  async healthCheck(): Promise<any> {
    ...
    try {
      ...
      return {
        ...
        models: 0,
        error: 'All Atria keys are currently in cooldown',
      };
    }
  ...
  async createEmbedding(_payload: any): Promise<any> {
    const err: any = new Error('Atria provider does not support embeddings.');
    err.status = 400;
    throw err;
  }
  ...
  private async makeRequest(method: string, url: string, data: any, apiKey: string, extraConfig?: AxiosRequestConfig): Promise<any> {
    ...
    logPipelineRequest({ provider: 'atria', baseUrl: this.baseUrl, endpoint: url, model, protocol: PROTOCOL, keyMasked: maskApiKey(apiKey) });
    const start = Date.now();
    ...
    logPipelineRaw({ provider: 'atria', model, endpoint: url, protocol: PROTOCOL, status: response.status, headers: response.headers, body: response.data, latencyMs: Date.now() - start });
  ...
  private async makeStreamRequest(url: string, data: any, apiKey: string): Promise<any> {
    ...
    logPipelineRaw({ provider: 'atria', model, endpoint: url, protocol: PROTOCOL, status: response.status, headers: response.headers, body: '[streaming]', latencyMs: Date.now() - start });
    return response.data;
  }
}
