...
export function createUnliKeyManager(keys: string[]): KeyManager {
  return new KeyManager(keys, 'UNLI');
}

function createAllKeysCooldownError(): any {
  const err: any = new Error('All UNLI API keys are currently in cooldown. Please wait before retrying.');
  err.status = 429;
...
function wrapError(error: any): any {
  ...
  const msg = errorBody?.error?.message ?? errorBody?.message ?? error?.message ?? 'UNLI API error';
  const err: any = new Error(`UNLI API error (${status}): ${msg}`);
  err.status = status;
  err.response = error?.response;
  return err;
}

export class UnliProvider implements Provider {
  ...
  private logSelection(endpoint: string, model: string, keyInfo: KeyInfo): void {
    console.log(`[UNLI] Base URL: ${this.baseUrl}`);
    console.log(`[UNLI] Endpoint: POST ${this.baseUrl}${endpoint}`);
    console.log(`[UNLI] Backend model: ${model}`);
    console.log(`[UNLI] API key index: KEY#${keyInfo.index + 1} (${maskKeySuffix(keyInfo.key)})`);
  }

  // UNLI (auto-routing) rejects reasoning_effort and thinking-related fields
  // with "Upstream API error". Strip them before forwarding.
  private sanitizePayload(payload: any): any {
    const clean = { ...payload };
    delete clean.reasoning_effort;
    delete clean.reasoning;
    delete clean.thinking;
    delete clean.thinking_budget;
    if ('max_tokens' in clean && typeof clean.max_tokens === 'number' && clean.max_tokens > 16384) {
  ...
  private async executeWithKey<T>(
    ...
    for (let attempt = 0; attempt < this.keyManager.keyCount; attempt++) {
      ...
      try {
        ...
        console.log(`[UNLI][KEY#${keyInfo.index + 1}] Success  Model=${model}  Latency=${latency}ms`);
        return result;
      } catch (error: any) {
        lastError = error;
        const latency = Date.now() - start;
        ...
        if (isQuotaError(error)) {
          ...
          logPipelineRateLimit('unli', model, COOLDOWN_DURATION_MS / 1000, status);
          console.log(`[UNLI][KEY#${keyInfo.index + 1}] Status=${status}  RateLimited  Cooldown=${COOLDOWN_DURATION_MS / 1000}s`);
        } else {
          this.keyManager.markFailure(keyInfo.index, error.message ?? String(error));
          console.log(`[UNLI][KEY#${keyInfo.index + 1}] Failed  Status=${status}  Error=${error.message ?? 'unknown'}`);
        }

        if (attempt < this.keyManager.keyCount - 1 && isRetryableError(error)) {
          console.log(`[UNLI] Retry with fresh key  Attempt=${attempt + 2}  Status=${status}  Model=${model}`);
          logPipelineError({ provider: 'unli', model, status, error: error.message ?? String(error), latencyMs: latency, retry: true });
          continue;
        }

        logPipelineError({ provider: 'unli', model, status, error: error.message ?? String(error), latencyMs: latency });
  ...
  async chatCompletion(payload: any): Promise<any> {
    ...
    logPipelineParsed('unli', clean.model, result, true);
    const { text, location } = findTextInResponse(result);
    logPipelineExtracted('unli', clean.model, text, location);
    logPipelineFinal('unli', clean.model, result);
    return result;
  }
  ...
  async chatCompletionRaw(payload: any): Promise<string> {
    ...
    logPipelineParsed('unli', clean.model, parsed.body, parsed.wasJson, parsed.parseError);
    const { text, location } = findTextInResponse(parsed.body ?? raw);
    logPipelineExtracted('unli', clean.model, text, location);
    logPipelineFinal('unli', clean.model, raw);
    return raw;
  }
  ...
  async chatCompletionStream(payload: any): Promise<{ stream: any; keyIndex: number; tag: string }> {
    ...
    for (let attempt = 0; attempt < this.keyManager.keyCount; attempt++) {
      ...
      try {
        ...
        logSuccessLatency(keyInfo.tag, latency);
        console.log(`[UNLI][KEY#${keyInfo.index + 1}] Success  Model=${model}  Latency=${latency}ms`);
        logPipelineFinal('unli', model, `[streaming] stream established`, latency);
  ...
  async listModels(): Promise<any> {
    ...
    try {
      ...
      console.warn('[UNLI] listModels: no key available — using last-known-good cache');
      const cached = discoveryStore.getLastGoodModels('unli');
      return {
        object: 'list',
        source: cached.length > 0 ? 'cache' : 'fallback',
        data: cached.length > 0 ? cached : FALLBACK_MODELS.map((id) => ({ id, object: 'model', created: Math.floor(now / 1000), owned_by: 'unli' })),
      };
    }
    ...
    const { outcome, models } = await runDiscovery({
      ...
      extract: openAIModelExtractor('unli'),
      onHealthy: (latency) => {
        this.keyManager.markSuccess(keyInfo.index, latency);
        logSuccessLatency(keyInfo.tag, latency);
      },
    ...
    // UNLI only ever advertises the auto-routing virtual model; merge the
    // known-good model names so routing stays usable for client requests.
    ...
    for (const id of FALLBACK_MODELS) {
      if (!merged.has(id)) {
        merged.set(id, { id, object: 'model', created: Math.floor(now / 1000), owned_by: 'unli', virtual: true });
      }
    }
  ...
  async healthCheck(): Promise<any> {
    ...
    try {
      ...
      return {
        ...
        models: 0,
        error: 'All UNLI API keys are currently in cooldown',
      };
    }
  ...
  async createEmbedding(_payload: any): Promise<any> {
    const err: any = new Error('UNLI provider does not support embeddings.');
    err.status = 400;
    throw err;
  }
  ...
  private async makeRequest(method: string, url: string, data: any, apiKey: string, extraConfig?: AxiosRequestConfig): Promise<any> {
    ...
    logPipelineRequest({ provider: 'unli', baseUrl: this.baseUrl, endpoint: url, model, protocol: PROTOCOL, keyMasked: maskApiKey(apiKey) });
    const start = Date.now();
    ...
    logPipelineRaw({ provider: 'unli', model, endpoint: url, protocol: PROTOCOL, status: response.status, headers: response.headers, body: response.data, latencyMs: Date.now() - start });
  ...
  private async makeStreamRequest(url: string, data: any, apiKey: string): Promise<any> {
    ...
    logPipelineRaw({ provider: 'unli', model, endpoint: url, protocol: PROTOCOL, status: response.status, headers: response.headers, body: '[streaming]', latencyMs: Date.now() - start });
    return response.data;
  }
}