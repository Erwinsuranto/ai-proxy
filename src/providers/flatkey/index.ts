...
const PROVIDER_INFO: ProviderInfo = {
  providerId: 'flatkey',
  providerName: 'FlatKey',
};

// Upstream endpoints (relative to baseUrl, e.g. https://router.flatkey.ai/v1).
...
export function createFlatKeyKeyManager(keys: string[]): KeyManager {
  return new KeyManager(keys, 'FlatKey');
}

function createAllKeysCooldownError(): any {
  const err: any = new Error('All FlatKey API keys are currently in cooldown. Please wait before retrying.');
  err.status = 429;
...
function wrapError(error: any): any {
  ...
  const msg = errorBody?.error?.message ?? errorBody?.message ?? error?.message ?? 'FlatKey API error';
  const err: any = new Error(`FlatKey API error (${status}): ${msg}`);
  err.status = status;
  err.response = error?.response;
  return err;
}

export class FlatKeyProvider implements Provider {
  ...
  private logSelection(endpoint: string, model: string, keyInfo: KeyInfo): void {
    console.log(`[FLATKEY] Base URL: ${this.baseUrl}`);
    console.log(`[FLATKEY] Endpoint: POST ${this.baseUrl}${endpoint}`);
    console.log(`[FLATKEY] Backend model: ${model}`);
    console.log(`[FLATKEY] API key index: KEY#${keyInfo.index + 1} (${maskKeySuffix(keyInfo.key)})`);
  }
  ...
  private async executeWithKey<T>(
    ...
    try {
      ...
      console.log(`[FLATKEY][KEY#${keyInfo.index + 1}] Success  Model=${model}  Latency=${latency}ms`);
      return result;
    } catch (error: any) {
      const latency = Date.now() - start;
      ...
      if (isQuotaError(error)) {
        ...
        logPipelineRateLimit('flatkey', model, COOLDOWN_DURATION_MS / 1000, status);
        console.log(`[FLATKEY][KEY#${keyInfo.index + 1}] Status=${status}  RateLimited  Cooldown=${COOLDOWN_DURATION_MS / 1000}s`);
      } else {
        this.keyManager.markFailure(keyInfo.index, error.message ?? String(error));
        console.log(`[FLATKEY][KEY#${keyInfo.index + 1}] Failed  Status=${status}  Error=${error.message ?? 'unknown'}`);
      }
      logPipelineError({ provider: 'flatkey', model, status, error: error.message ?? String(error), latencyMs: latency });
  ...
  async chatCompletion(payload: any): Promise<any> {
    ...
    logPipelineParsed('flatkey', payload.model, result, true);
    const { text, location } = findTextInResponse(result);
    logPipelineExtracted('flatkey', payload.model, text, location);
    logPipelineFinal('flatkey', payload.model, result);
    return result;
  }
  ...
  async chatCompletionRaw(payload: any): Promise<string> {
    ...
    logPipelineParsed('flatkey', payload.model, parsed.body, parsed.wasJson, parsed.parseError);
    const { text, location } = findTextInResponse(parsed.body ?? raw);
    logPipelineExtracted('flatkey', payload.model, text, location);
    logPipelineFinal('flatkey', payload.model, raw);
    return raw;
  }
  ...
  async chatCompletionStream(payload: any): Promise<{ stream: any; keyIndex: number; tag: string }> {
    ...
    try {
      ...
      logSuccessLatency(keyInfo.tag, latency);
      console.log(`[FLATKEY][KEY#${keyInfo.index + 1}] Success  Model=${model}  Latency=${latency}ms`);
      logPipelineFinal('flatkey', model, `[streaming] stream established`, latency);
  ...
  async listModels(): Promise<any> {
    ...
    try {
      ...
      console.warn('[FLATKEY] listModels: no key available — using last-known-good cache');
      const cached = discoveryStore.getLastGoodModels('flatkey');
      return {
        object: 'list',
        source: cached.length > 0 ? 'cache' : 'fallback',
        data: cached.length > 0 ? cached : FALLBACK_MODELS.map((id) => ({ id, object: 'model', created: Math.floor(now / 1000), owned_by: 'flatkey' })),
      };
    }
    ...
    const { outcome, models } = await runDiscovery({
      ...
      extract: openAIModelExtractor('flatkey'),
      onHealthy: (latency) => {
        this.keyManager.markSuccess(keyInfo.index, latency);
        logSuccessLatency(keyInfo.tag, latency);
      },
    ...
    return {
      object: 'list',
      source: 'fallback',
      data: FALLBACK_MODELS.map((id) => ({ id, object: 'model', created: Math.floor(now / 1000), owned_by: 'flatkey' })),
    };
  }
  ...
  async healthCheck(): Promise<any> {
    ...
    try {
      ...
      return {
        ...
        models: 0,
        error: 'All FlatKey API keys are currently in cooldown',
      };
    }
  ...
  async createEmbedding(_payload: any): Promise<any> {
    const err: any = new Error('FlatKey provider does not support embeddings.');
    err.status = 400;
    throw err;
  }
  ...
  private async makeRequest(method: string, url: string, data: any, apiKey: string, extraConfig?: AxiosRequestConfig): Promise<any> {
    ...
    logPipelineRequest({ provider: 'flatkey', baseUrl: this.baseUrl, endpoint: url, model, protocol: PROTOCOL, keyMasked: maskApiKey(apiKey) });
    const start = Date.now();
    ...
    logPipelineRaw({ provider: 'flatkey', model, endpoint: url, protocol: PROTOCOL, status: response.status, headers: response.headers, body: response.data, latencyMs: Date.now() - start });
    // Fix #1: HTTP 200 but the body is actually an inline upstream error.
    const inline = detectInlineUpstreamError(response.data);
    if (inline) {
      throw createUpstreamError('FlatKey', inline);
    }
  ...
  private async makeStreamRequest(url: string, data: any, apiKey: string): Promise<any> {
    ...
    const response = await this.client.post(url, data, {
      headers: this.buildHeaders(apiKey),
      responseType: 'stream',
      // Fix #2: FlatKey requests must not be cut off at the global 30s timeout.
      // Apply the FlatKey-specific (120s) timeout consistently to normal AND
      // streaming requests so long generations can complete.
      timeout: this.timeout,
    } as AxiosRequestConfig);
    logPipelineRaw({ provider: 'flatkey', model, endpoint: url, protocol: PROTOCOL, status: response.status, headers: response.headers, body: '[streaming]', latencyMs: Date.now() - start });
  ...
  private inspectStreamStart(upstream: any): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      const settle = (verdict: StreamVerdict) => {
        ...
        cleanup();
        if (verdict === 'error') {
          const err = createUpstreamError('FlatKey', {
            status: 502,
