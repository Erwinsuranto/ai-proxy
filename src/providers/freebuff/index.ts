...
const PROVIDER_INFO: ProviderInfo = {
  providerId: 'freebuff',
  providerName: 'Freebuff (Codebuff Free)',
};
...
export function createFreebuffKeyManager(keys: string[]): KeyManager {
  return new KeyManager(keys, 'Freebuff');
}

function createAllKeysCooldownError(): any {
  const err: any = new Error('All Freebuff API keys are currently in cooldown. Please wait before retrying.');
  err.status = 429;
...
function wrapError(error: any): any {
  ...
  const msg = errorBody?.error?.message ?? errorBody?.message ?? error?.message ?? 'Freebuff API error';
  const err: any = new Error(`Freebuff API error (${status}): ${msg}`);
  err.status = status;
...
export class FreebuffProvider implements Provider {
  ...
  private logSelection(endpoint: string, model: string, keyInfo: KeyInfo): void {
    console.log(`[FREEBUFF] Base URL: ${this.baseUrl}`);
    console.log(`[FREEBUFF] Endpoint: POST ${this.baseUrl}${endpoint}`);
    console.log(`[FREEBUFF] Backend model: ${model}`);
    console.log(`[FREEBUFF] API key index: KEY#${keyInfo.index + 1} (${maskKeySuffix(keyInfo.key)})`);
  }
  ...
  private async executeWithKey<T>(
    ...
    try {
      ...
      console.log(`[FREEBUFF][KEY#${keyInfo.index + 1}] Success  Model=${model}  Latency=${latency}ms`);
      return result;
    } catch (error: any) {
      const latency = Date.now() - start;
      ...
      if (isQuotaError(error)) {
        ...
        logPipelineRateLimit('freebuff', model, COOLDOWN_DURATION_MS / 1000, status);
        console.log(`[FREEBUFF][KEY#${keyInfo.index + 1}] Status=${status}  RateLimited  Cooldown=${COOLDOWN_DURATION_MS / 1000}s`);
      } else {
        this.keyManager.markFailure(keyInfo.index, error.message ?? String(error));
        console.log(`[FREEBUFF][KEY#${keyInfo.index + 1}] Failed  Status=${status}  Error=${error.message ?? 'unknown'}`);
      }
      logPipelineError({ provider: 'freebuff', model, status, error: error.message ?? String(error), latencyMs: latency });
  ...
  async chatCompletion(payload: any): Promise<any> {
    ...
    logPipelineParsed('freebuff', payload.model, result, true);
    const { text, location } = findTextInResponse(result);
    logPipelineExtracted('freebuff', payload.model, text, location);
    logPipelineFinal('freebuff', payload.model, result);
    return result;
  }
  ...
  async chatCompletionRaw(payload: any): Promise<string> {
    ...
    logPipelineParsed('freebuff', payload.model, parsed.body, parsed.wasJson, parsed.parseError);
    const { text, location } = findTextInResponse(parsed.body ?? raw);
    logPipelineExtracted('freebuff', payload.model, text, location);
    logPipelineFinal('freebuff', payload.model, raw);
    return raw;
  }
  ...
  async chatCompletionStream(payload: any): Promise<{ stream: any; keyIndex: number; tag: string }> {
    ...
    try {
      ...
      logSuccessLatency(keyInfo.tag, latency);
      console.log(`[FREEBUFF][KEY#${keyInfo.index + 1}] Success  Model=${model}  Latency=${latency}ms`);
      logPipelineFinal('freebuff', model, `[streaming] stream established`, latency);
  ...
  async listModels(): Promise<any> {
    ...
    try {
      ...
      console.warn('[FREEBUFF] listModels: no key available — using last-known-good cache');
      const cached = discoveryStore.getLastGoodModels('freebuff');
      return {
        object: 'list',
        source: cached.length > 0 ? 'cache' : 'fallback',
        data: cached.length > 0 ? cached : FALLBACK_MODELS.map((id) => ({ id, object: 'model', created: Math.floor(now / 1000), owned_by: 'freebuff' })),
      };
    }
    ...
    const { outcome, models } = await runDiscovery({
      ...
      extract: openAIModelExtractor('freebuff'),
      onHealthy: (latency) => {
        this.keyManager.markSuccess(keyInfo.index, latency);
        logSuccessLatency(keyInfo.tag, latency);
      },
    ...
    return {
      object: 'list',
      source: 'fallback',
      data: FALLBACK_MODELS.map((id) => ({ id, object: 'model', created: Math.floor(now / 1000), owned_by: 'freebuff' })),
    };
  }
  ...
  async healthCheck(): Promise<any> {
    ...
    try {
      ...
      return {
        ...
        models: 0,
        error: 'All Freebuff API keys are currently in cooldown',
      };
    }
  ...
  async createEmbedding(_payload: any): Promise<any> {
    const err: any = new Error('Freebuff provider does not support embeddings.');
    err.status = 400;
    throw err;
  }
  ...
  private async makeRequest(method: string, url: string, data: any, apiKey: string, extraConfig?: AxiosRequestConfig): Promise<any> {
    ...
    logPipelineRequest({ provider: 'freebuff', baseUrl: this.baseUrl, endpoint: url, model, protocol: PROTOCOL, keyMasked: maskApiKey(apiKey) });
    const start = Date.now();
    ...
    logPipelineRaw({ provider: 'freebuff', model, endpoint: url, protocol: PROTOCOL, status: response.status, headers: response.headers, body: response.data, latencyMs: Date.now() - start });
  ...
  private async makeStreamRequest(url: string, data: any, apiKey: string): Promise<any> {
    ...
    logPipelineRaw({ provider: 'freebuff', model, endpoint: url, protocol: PROTOCOL, status: response.status, headers: response.headers, body: '[streaming]', latencyMs: Date.now() - start });
    return response.data;
  }
}