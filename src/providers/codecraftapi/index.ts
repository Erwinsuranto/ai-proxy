...
const PROVIDER_INFO: ProviderInfo = {
  providerId: 'codecraftapi',
  providerName: 'CodeCraft API',
};
...
export function createCodeCraftApiKeyManager(keys: string[]): KeyManager {
  return new KeyManager(keys, 'CodeCraft API');
}

function createAllKeysCooldownError(): any {
  const err: any = new Error('All CodeCraft API keys are currently in cooldown. Please wait before retrying.');
  err.status = 429;
...
function wrapError(error: any): any {
  ...
  ) ?? errorBody?.message ?? error?.message ?? 'CodeCraft API error';
  const err: any = new Error(`CodeCraft API error (${status}): ${msg}`);
  err.status = status;
  err.response = error?.response;
  return err;
}

export class CodeCraftApiProvider implements Provider {
  ...
  private logSelection(endpoint: string, model: string, keyInfo: KeyInfo): void {
    console.log(`[CODECRAFT] Base URL: ${this.baseUrl}`);
    console.log(`[CODECRAFT] Endpoint: POST ${this.baseUrl}${endpoint}`);
    console.log(`[CODECRAFT] Backend model: ${model}`);
    console.log(`[CODECRAFT] API key index: KEY#${keyInfo.index + 1} (${maskKeySuffix(keyInfo.key)})`);
  }
  ...
  private async executeWithKey<T>(
    ...
    for (let attempt = 0; attempt < this.keyManager.keyCount; attempt++) {
      ...
      try {
        ...
        console.log(`[CODECRAFT][KEY#${keyInfo.index + 1}] Success  Model=${model}  Latency=${latency}ms`);
        return result;
      } catch (error: any) {
        lastError = error;
        const latency = Date.now() - start;
        ...
        if (isQuotaError(error)) {
          ...
          logPipelineRateLimit('codecraftapi', model, COOLDOWN_DURATION_MS / 1000, status);
          console.log(`[CODECRAFT][KEY#${keyInfo.index + 1}] Status=${status}  RateLimited  Cooldown=${COOLDOWN_DURATION_MS / 1000}s`);
        } else {
          this.keyManager.markFailure(keyInfo.index, error.message ?? String(error));
          console.log(`[CODECRAFT][KEY#${keyInfo.index + 1}] Failed  Status=${status}  Error=${error.message ?? 'unknown'}`);
        }

        if (attempt < this.keyManager.keyCount - 1 && isRetryableError(error)) {
          console.log(`[CODECRAFT] Retry with fresh key  Attempt=${attempt + 2}  Status=${status}  Model=${model}`);
          logPipelineError({ provider: 'codecraftapi', model, status, error: error.message ?? String(error), latencyMs: latency, retry: true });
          continue;
        }

        logPipelineError({ provider: 'codecraftapi', model, status, error: error.message ?? String(error), latencyMs: latency });
  ...
  async chatCompletion(payload: any): Promise<any> {
    ...
    logPipelineParsed('codecraftapi', payload.model, result, true);
    const { text, location } = findTextInResponse(result);
    logPipelineExtracted('codecraftapi', payload.model, text, location);
    logPipelineFinal('codecraftapi', payload.model, result);
    return result;
  }
  ...
  async chatCompletionRaw(payload: any): Promise<string> {
    ...
    logPipelineParsed('codecraftapi', payload.model, parsed.body, parsed.wasJson, parsed.parseError);
    const { text, location } = findTextInResponse(parsed.body ?? raw);
    logPipelineExtracted('codecraftapi', payload.model, text, location);
    logPipelineFinal('codecraftapi', payload.model, raw);
    return raw;
  }
  ...
  async chatCompletionStream(payload: any): Promise<{ stream: any; keyIndex: number; tag: string }> {
    ...
    for (let attempt = 0; attempt < this.keyManager.keyCount; attempt++) {
      ...
      try {
        ...
        logSuccessLatency(keyInfo.tag, latency);
        console.log(`[CODECRAFT][KEY#${keyInfo.index + 1}] Success  Model=${model}  Latency=${latency}ms`);
        logPipelineFinal('codecraftapi', model, `[streaming] stream established`, latency);
  ...
  async listModels(): Promise<any> {
    ...
    try {
      ...
      console.warn('[CODECRAFT] listModels: no key available — using last-known-good cache');
      const cached = discoveryStore.getLastGoodModels('codecraftapi');
      return {
        object: 'list',
        source: cached.length > 0 ? 'cache' : 'fallback',
        data: cached.length > 0 ? cached : FALLBACK_MODELS.map((id) => ({ id, object: 'model', created: Math.floor(now / 1000), owned_by: 'codecraftapi' })),
      };
    }
    ...
    const { outcome, models } = await runDiscovery({
      ...
      extract: openAIModelExtractor('codecraftapi'),
      onHealthy: (latency) => {
        this.keyManager.markSuccess(keyInfo.index, latency);
        logSuccessLatency(keyInfo.tag, latency);
      },
    ...
    return {
      object: 'list',
      source: 'fallback',
      data: FALLBACK_MODELS.map((id) => ({ id, object: 'model', created: Math.floor(now / 1000), owned_by: 'codecraftapi' })),
    };
  }
  ...
  async healthCheck(): Promise<any> {
    ...
    try {
      ...
      return {
        ...
        models: 0,
        error: 'All CodeCraft API keys are currently in cooldown',
      };
    }
  ...
  async createEmbedding(_payload: any): Promise<any> {
    const err: any = new Error('CodeCraft API provider does not support embeddings.');
    err.status = 400;
    throw err;
  }
  ...
  private async makeRequest(method: string, url: string, data: any, apiKey: string, extraConfig?: AxiosRequestConfig): Promise<any> {
    ...
    logPipelineRequest({ provider: 'codecraftapi', baseUrl: this.baseUrl, endpoint: url, model, protocol: PROTOCOL, keyMasked: maskApiKey(apiKey) });
    const start = Date.now();
    ...
    logPipelineRaw({ provider: 'codecraftapi', model, endpoint: url, protocol: PROTOCOL, status: response.status, headers: response.headers, body: response.data, latencyMs: Date.now() - start });
  ...
  private async makeStreamRequest(url: string, data: any, apiKey: string): Promise<any> {
    ...
    logPipelineRaw({ provider: 'codecraftapi', model, endpoint: url, protocol: PROTOCOL, status: response.status, headers: response.headers, body: '[streaming]', latencyMs: Date.now() - start });
    return response.data;
  }
}