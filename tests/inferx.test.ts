...
import { InferXProvider, createInferXKeyManager } from '../src/providers/inferx';

const BASE_URL = 'https://model.inferx.net/endpoints/v1';

function makeProvider(): InferXProvider {
  const km = createInferXKeyManager(['test-key-1', 'test-key-2']);
  return new InferXProvider(km, BASE_URL, 10000);
}

function mockClient(provider: InferXProvider, overrides: Record<string, any> = {}): void {
...
describe('InferXProvider', () => {
  it('getProviderInfo returns inferx identity', () => {
    const provider = makeProvider();
    expect(provider.getProviderInfo()).toEqual({ providerId: 'inferx', providerName: 'InferX' });
  });
  ...
  it('listModels falls back to empty catalog when upstream fails', async () => {
    vi.resetModules();
    const { InferXProvider, createInferXKeyManager } = await import('../src/providers/inferx');
    const km = createInferXKeyManager(['test-key-1', 'test-key-2']);
    const provider = new InferXProvider(km, BASE_URL, 10000);
    mockClient(provider, {
  ...
  it('healthCheck reports ok with model count when GET /models succeeds', async () => {
    ...
    expect(health.provider).toBe('inferx');
